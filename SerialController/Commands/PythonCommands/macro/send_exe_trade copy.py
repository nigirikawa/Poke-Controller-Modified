    ...
    def send_trade(self):
        ...
        while True:
            ...
            if not self.isContainTemplate("Macro/rokkuman_exe/trade_chip_frame.png", crop=[203, 134, 224, 300]):
                ...
                # len(image): 720 y 
                # len(image[0]): 1280 x
                if self.isContainTemplate("Macro/rokkuman_exe/no_data_check.png", crop=[250, 120, 450, 290]):
                    ...
                    while True:
                        ...
                        if self.isContainTemplate("Macro/rokkuman_exe/choice_of_trading_partner.png", crop=[600, 200, 620, 220]):
                            self._logger.info("申込先を選択する")
                            while True:
                                # Snipping(564, 455, 770, 290)
                                if not self.isContainTemplate("Macro/rokkuman_exe/trade_application_confirmation.png", crop=[420, 325, 880, 500]):
                                    self._logger.info("申し込み確定")
                                    self.press(Button.A, 0.2, 0.2)
                                    break
                            while True:
                                # Snipping(564, 455, 770, 290)
                                if self.isContainTemplate("Macro/rokkuman_exe/trade_application_confirmation.png", crop=[420, 325, 880, 500]):
                                    self._logger.info("申し込み確定")
                                    self.press(Button.A, 0.2, 0.2)
                                    break
                            while True:
                                if self.isContainTemplate("Macro/rokkuman_exe/successful_trade.png", crop=[550, 300, 740, 480]):
                                    # OK　選択
                                    ...
                                    while True:
                                        ...
                                        if self.isContainTemplate("Macro/rokkuman_exe/network_initial_screen.png", crop=[120, 120, 400, 150]):
                                            return
                        else:
                            # Snipping(586, 227, 685, 487)
                            if self.isContainTemplate("Macro/rokkuman_exe/no_communication_partner_update_list.png", crop=[410, 170, 830, 500]):
